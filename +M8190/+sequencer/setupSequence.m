function result = setupSequence(f, seqcmd, varargin)
    ...
    switch (seqcmd.cmd)
        case 'list'
            ...
            if (s(1,1) == 0)
                ...
                if ~(isfield(seqcmd, 'quiet') && seqcmd.quiet)
                    msgbox({sprintf('The following segments are defined:%s', ...
                        sprintf(' %d', s(1,:)))}, 'Segments list');
                end
                result = s(1,:);
            end
        case 'delete'
...
%            xfprintf(f, sprintf(':STABle%d:DATA 0%s', i, cmd));
            M8190.xfprintf(f, sprintf(':STABle%d:SEQuence:SELect %d', i, 0));
            M8190.xfprintf(f, sprintf(':STABle%d:DYNamic:STATe 0', i));
            M8190.xfprintf(f, sprintf(':FUNCtion%d:MODE STSequence', i));
        end
    end
...
% Action:     Control / Seq.Loops / Cmd Code(1) + Act.ID / Segm.ID / Start Offset / End Offset
    ...
    if (seqline.segmentNumber == 0)           % segment# = 0 means: idle command
        ...
        if (isfield(seqline, 'actionID')&& ~isempty(seqline.actionID) && seqline.actionID >= 0)
            ...
            if (isfield(seqline, 'segmentLoops') && ~isempty(seqline.segmentLoops) && seqline.segmentLoops > 1)% i did 1
