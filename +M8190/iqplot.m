...
% plot I/Q waveform, spectrum and spectrogram
%
...
if (isempty(find(strcmp(varargin, 'no_timedomain'))))
    figure(1);
%    clf(gcf(), 'reset');
    ...
    if (complexData)
        leg = {['I (CF=' sprintf('%.2g', crest(re)) ' dB)'], ['Q (CF=' sprintf('%.2g', crest(im)) ' dB)']};
    else
        leg = {['Data (CF=' sprintf('%.2g', crest(re)) ' dB)']};
    end
    xlabel(timeStr);
    ylabel('AU');
